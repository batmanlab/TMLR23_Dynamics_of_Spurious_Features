...
class DenseNet(nn.Module):
    ...
    def forward(self, x):
        ...
        features = F.dropout(features, p=self.drop_rate, training=True) #trainging is always true as drop_rate!=0 means MCDropout
...
class customDenseNet(nn.Module):
    ...
    def __init__(self, train_embs_path, pd, growth_rate=32, block_config=(6, 12, 24, 16), num_init_features=64, bn_size=4,
                 drop_rate=0, num_classes=18, in_channels=1, weights=None, op_threshs=None, progress=True,
                 apply_sigmoid=False, return_logit=False):
    ...
    def get_nbr_info(self, x):
        ...
        test_feat = x
        test_feat = torch.nn.functional.interpolate(test_feat,(8,8))
